...
with torch.cuda.device('cuda:0'):
    ...
    for lam_fair in lfs:
        for seed in seeds:
            if os.path.exists(f"../model_results_celeba-fairsurrogates_{form}_abs/model_{form}_{lam_fair}_{seed}_eo.pth"):
                model.load_state_dict(torch.load(f"../model_results_celeba-fairsurrogates_{form}_abs/model_{form}_{lam_fair}_{seed}_eo.pth"))
    ...
    df.to_csv(f"./celeba_{form}_eo_test.csv", index=False)